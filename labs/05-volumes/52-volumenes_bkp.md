...
- Crea un volumen llamado data_vol y un contenedor que monte este volumen en `/app`:
    ...
    docker run -it --rm --name app_container -v data_vol:/app alpine
    ```

    - Tener en cuenta que el contenedor se ejecuta con `--rm`, por lo que se eliminará automáticamente al salir.
...
- Sal del contenedor:
    ...
    exit
    ```
- El contenedor se habrá detenido y eliminado automáticamente.
...
- Para comprobarlo realizar el siguiente comando:

    ```bash
    ls
    ```

### Desglosando el comando `docker run`
...
- `tar czf /backup/backup_data_vol.tar.gz -C /data .`:
    - `tar`: es una herramienta de Unix que se utiliza para crear archivos del tipo `tarball`, que son básicamente archivos empaquetados en uno solo.
...
- Abre otra ventana de PowerShell o Bash en tu host crea un archivo llamado `nuevos_datos.txt` en la carpeta `workdir`con el contenido "Nuevos datos para Docker". Lo podemos crear con el siguiente comando:

    ```bash
    echo "Nuevos datos para Docker" > nuevos_datos.txt
    ```
...
Dentro del contenedor, en la ventana de PowerShell o Bash que abrimos para crear el contenedor `restored_container`, verifica que el archivo ha sido copiado correctamente:

```bash
cat /app/nuevos_datos.txt
```

O en una ventana cualquier podés ejecutar:

```bash
docker exec restored_container cat /app/nuevos_datos.txt
```
...
- Listar los volúmenes para verificar que han sido eliminados:

```bash
docker volume ls
```

Si alguno no se pudo eliminar, es probable que algún contenedor lo esté utilizando. Asegúrate de detener y eliminar cualquier contenedor que esté usando esos volúmenes antes de intentar eliminarlos nuevamente.
