...
## 1. Preparación:
  ...
  - Crear un contenedor llamado `dbstore` que monte este volumen usando la sintaxis `--mount`:
    ...
    > Usamos tail -f /dev/null para que el contenedor no se detenga.
...
En este laboratorio aprendimos a realizar backups y migraciones de volúmenes en Docker. Vimos como crear un volumen, realizar un backup y restaurarlo en un nuevo volumen.
