...
- **Ubuntu:** Distribución muy popular basada en Debian, conocida por su facilidad de uso, amplia comunidad de soporte, y aplicaciones tanto para escritorio como para servidores.
...
Para este laboratorio elija una distribución de Linux para los siguientes ejercicios.
...
- Para Ubuntu:
    ```bash
    docker run -it --name contenedor-linux ubuntu
    ```

- Para AlmaLinux:

    ```bash
    docker run -it --name contenedor-linux almalinux bash
    ```

- Para AlpineLinux:

    ```bash
    docker run -it --name contenedor-linux alpine sh
    ```
...
1. **Guarda el contenedor como una nueva imagen con la etiqueta `v1.0`:**
    ...
    ```bash
    docker commit -m "Instalamos curl" contenedor-linux mi-imagen:v1.0 
    ```
...
3. **Verificar la capa nueva en la imágen:**

    ```bash
    docker image history mi-imagen:v1.0
    ```

    Tiene que ver algo similar a esto

    ```bash
    IMAGE          CREATED          CREATED BY                                      SIZE      COMMENT
    ee29a5405b5d   28 seconds ago   /bin/bash                                       72.1MB    Instalamos curl
    ce8f79aecc43   6 weeks ago      /bin/sh -c #(nop)  CMD ["/bin/bash"]            0B        
    <missing>      6 weeks ago      /bin/sh -c #(nop) ADD file:d9cb8116905a82675…   78.1MB    
...
## Resumen de Laboratorio 2.3

En este laboratorio hemos realizado lo siguiente:
