...
Desde una consola de PowerShell o Bash para prepararnos para iniciar un contenedor Ubuntu con el siguiente comando. Pero todavía no lo ejecutemos.

```bash
docker run -d --name duerme_30 ubuntu sleep 30
```

Analicemos antes qué va a hacer el comando:
...
- Ahora sí ejecutamos el comando preparado anteriormente:
