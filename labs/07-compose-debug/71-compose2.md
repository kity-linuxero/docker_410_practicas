# Laboratorio 7.1 - Docker Compose | Parte II
...
- Crear un archivo llamado `.env` (debe llamarse tal cual así) en la raíz de tu proyecto para definir las variables de entorno con el siguiente contenido.
...
- Crear un archivo `compose.yml` en la carpeta raíz del proyecto con el siguiente contenido
    ...
    > Observar que no se ha especificado el nombre del archivo de variables de entorno. Eso es porque el archivo se llama `.env`, que es el que busca por defecto Docker Compose.
### Explicación de los conceptos:

- **Interpolation:**
    - `${APP_NAME}`: se usa para asignar un nombre dinámico al contenedor.
    - `${APP_PORT}`: se usa para utilizar un puerto del que escucha el host. Se cambia desde el archivo `.env`.
...
## 8. Detener los servicios y limpieza
...
En este lab hemos aprendido a utilizar variables de entorno en Docker Compose para configurar los servicios.
