# Laboratorio 6.2 - Aplicaciones multicontenedor
...
Para este ejercicio, el desarrollador nos provee de su repositorio Git donde tiene alojada la aplicación. La aplicación consiste en un proxy reverso Nginx que distribuye las solicitudes entre dos servidores web Node.js, los cuales almacenan el conteo de visitas en una base de datos Redis.

Esquema de la app:

![](./img/multicontainer.png)



Si tenemos git instalado ejecutamos:

```bash
git clone https://github.com/kity-linuxero/nginx-node-redis.git
```

- Si no tenemos git instalado, podemos descargar el <a href="https://github.com/kity-linuxero/nginx-node-redis/archive/refs/heads/main.zip" download>ZIP</a> del repositorio.

- Una vez descargada y descomprimida, nos ubicamos con PowerShell o Bash en el directorio de la aplicación `nginx-node-redis`.
...
- En la carpeta raíz del proyecto hay un archivo llamado `compose.yml` con el siguiente contenido:
    ...
    # Proxy reverso con Nginx
      nginx:
        build: ./nginx
        # Se publica el puerto 80 hacia afuera del host
        ports:
          - '80:80'
        depends_on:
        - web1
...
El comando realizará el build de las imágenes y levantará los contenedores definidos en el archivo `compose.yml`.
...
```bash
Running 5/5
 ✔ Network nginx-node-redis_default    Created                                                                                   0.1s
 ✔ Container nginx-node-redis-web1-1   Started                                                                                   0.9s
 ✔ Container nginx-node-redis-web2-1   Started                                                                                   0.8s
 ✔ Container nginx-node-redis-redis-1  Started                                                                                   1.0s
 ✔ Container nginx-node-redis-nginx-1  Started                                                                                   1.2s
```
