# Laboratorio 3.3 - La mejor página
...
ea914d8b91b3  best-page  "nginx -g 'daemon of…"  5 seconds ago  Up 5 seconds  80/tcp              web
```
...
Podemos detener el contenedor de varias maneras. Una de ellas es usar el comando `docker stop` seguido del `nombre` o `ID` del contenedor o bien eliminarlo directamente, forzando la eliminación con el parámetro `-f`.
...
```bash
docker run -d --name web -p 80:80 best-page:alpine
```
